...
def _capacity_for_class(socket_class: str) -> float:
    if socket_class == "main":
        return 0.55
    if socket_class == "retro":
        return 0.35
    return 0.22
...
def _socket_record_to_manifest(record: base.SocketRecord) -> dict[str, Any]:
    ...
    return {
        ...
        "capacity": _capacity_for_class(record.socket_class),
    }


def _bounds_blender_to_godot(
    minimum: Vector,
    maximum: Vector,
...
def _effect_record_to_manifest(
    record: NozzleLocalEffectGeometryRecord,
) -> dict[str, Any]:
    bounds_min, bounds_max = _bounds_blender_to_godot(
    ...
    return {
        ...
        "node_transform_basis": base.blender_basis_to_godot_rows(
            record.node_transform_basis_blender
        ),
...
def write_schema_four_manifest(
    path: Path,
    output_path: Path,
    source_path: Path,
    ...
    if not path.is_file() or path.stat().st_size <= 0:
        raise RuntimeError(f"Schema-4 fighter manifest missing or empty: {path}")
...
def main() -> None:
    ...
    if base.sha256_file(source_path) != actual_sha:
        raise RuntimeError("Source SHA changed during schema-4 canonical export")
    ...
    print(f"Maximum reconstruction error: {maximum_error:.12f} m")
    print(f"Canonical GLB: {output_path}")
    print(f"Schema-4 manifest: {manifest_path}")
